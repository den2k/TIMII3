...
/* --- TODO Section ---
 ...
 TODO: 1.13.19 [DONE 1.13.19] - Added isEmpty error checking of username and Full name
...
class CreateAccountScreen: UIViewController, LayoutLoading
{
    ...
    {
        guard let email = emailTextField?.text, !email.isEmpty, let password = passwordTextField?.text, !password.isEmpty, let fullName = fullNameTextField?.text, !fullName.isEmpty, let userName = userNameTextField?.text, !userName.isEmpty else
        {
        ...
        Auth.auth().createUser(withEmail: email, password: password, completion:
            {(user, error) in
                ...
                // Post notification that member was created and login? and dismiss to Main screen
                print("--> New Member created: \(newMember)")
                NotificationCenter.default.post(name: .didMemberLogin, object: nil)
                self.presentingViewController?.presentingViewController?.dismiss(animated: true, completion: nil) // Dismiss both present CreateAccount VC and Login VC to arrive at Main
